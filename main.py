...
with introduction:
    ...
    st.text("This application was created to help guide users to perform time series analysis as well as forecasting using U.S Economic data provided by the Federal Bank of St. Louis.")
...
with landing_page_eda:
    ...
    fig_all_indicators.update_layout(xaxis_range=[economic_df_filtered.iloc[0]['Date'],economic_df_filtered.iloc[-1]['Date']],
                    title_text="Monthly U.S Economic data over time",
    ...
    # Viz
    df_corr_viz = correlation.mask(mask).dropna('columns', how='all')
    ...
    fig_correlation.update_layout(title_text = 'Correlation plot among the economic indicators')
    # fig.show()
    correlation_plot.plotly_chart(fig_correlation)
    
    st.write('Make sure to have at least 3 years of data before selecting one of the following options')
    ...
    if perform_advanced_eda:
        ...
        if user_selected_indicator:
            ...
            fig_w_traces.add_traces(go.Scatter(x=top_dates['Date'], y=top_dates[user_selected_indicator],
                                                        ...
                                                        text = top_dates["tgl"]))
            fig_w_traces.update_layout(template="simple_white", font=dict(size=18), title_text='Bar chart of economic indicator over time',
            ...
            fig_w_traces.add_traces(go.Scatter(x=top_dates['Date'], y=top_dates[user_selected_indicator],
                            ...
                            # fig_w_traces.show()
            fig_w_traces.update_layout(template="simple_white", font=dict(size=18), title_text='Line chart of economic indicator over time',
                                width=650, title_x=0.5, height=400, 
                                xaxis_title='Date',
                                yaxis_title='Percent')
            ...
            smoothed_trend.text('Smoothing the data with a rolling average')
            ...
            window_selection = smoothed_trend.selectbox('Choose a window(months) to use for the rolling average', window_options)
